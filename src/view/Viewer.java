...
public class Viewer extends JPanel implements Runnable {
    ...
    public Viewer(View view) {
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                Point p = e.getPoint();
                List<Ball> balls = view.getAllBalls();
                Ball found = null;
                for (Ball b : balls) {
                    double bx = b.getX();
                    double by = b.getY();
                    int d = b.getDIAMETER();
                    Rectangle r = new Rectangle((int)bx, (int)by, d, d);
                    if (r.contains(p)) { found = b; break; }
                }
                selectedBall = found;
        ...
        // Key bindings for WASD to control the selected ball
        ...
        // accelerate (W) -> increase speed by 10% (single press)
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_W, 0, false), "speedUp");
        am.put("speedUp", new AbstractAction() { @Override public void actionPerformed(java.awt.event.ActionEvent e) { if (selectedBall != null) selectedBall.scaleSpeed(1.10); } });
        // decelerate (S) -> reduce speed by 10% (single press)
        im.put(KeyStroke.getKeyStroke(KeyEvent.VK_S, 0, false), "slowDown");
        am.put("slowDown", new AbstractAction() { @Override public void actionPerformed(java.awt.event.ActionEvent e) { if (selectedBall != null) selectedBall.scaleSpeed(0.90); } });
    // Deselect on Escape
    ...
    am.put("deselect", new AbstractAction() { @Override public void actionPerformed(java.awt.event.ActionEvent e) { selectedBall = null; Viewer.this.repaint(); } });
        ...
        am.put("selectNext", new AbstractAction() {
            @Override public void actionPerformed(java.awt.event.ActionEvent e) {
                ...
                if (selectedBall == null) {
                    selectedBall = balls.get(0);
                } else {
                    int idx = balls.indexOf(selectedBall);
                    if (idx < 0) {
                        selectedBall = balls.get(0);
                    } else {
                        int next = (idx + 1) % balls.size();
                        selectedBall = balls.get(next);
                    }
                }
                Viewer.this.requestFocusInWindow();
                Viewer.this.repaint();
        ...
        am.put("selectPrev", new AbstractAction() {
            @Override public void actionPerformed(java.awt.event.ActionEvent e) {
                ...
                if (balls == null || balls.isEmpty()) return;
                if (selectedBall == null) {
                    selectedBall = balls.get(balls.size() - 1);
                } else {
                    int idx = balls.indexOf(selectedBall);
                    if (idx < 0) {
                        selectedBall = balls.get(balls.size() - 1);
    ...
    protected void paintComponent(Graphics g) {
        ...
        model.Habitacion hab = view.getHabitacion();
        if (hab != null) {
    ...
    public void run() {
        ...
        while (running) {
            ...
            last = now;
            // apply continuous rotation based on flags (rotate by speed * dt)
            Ball sel = selectedBall;
            if (sel != null) {
                if (rotatingLeft) sel.rotate(-ROTATION_RAD_PER_SEC * dtSeconds);
                if (rotatingRight) sel.rotate(ROTATION_RAD_PER_SEC * dtSeconds);
            }
    ...
    public void paintBall(Ball ball, Graphics2D g) {
        ...
        // if this ball is currently selected, draw a triangle pointing in the velocity direction
        if (ball == selectedBall) {
            // compute center
            double cx = x + diameter / 2.0;
            double cy = y + diameter / 2.0;
            double vx = ball.getVelX();
            double vy = ball.getVelY();
            double speed = Math.hypot(vx, vy);
            // default direction if stationary
            double angle = (speed > 1e-8) ? Math.atan2(vy, vx) : -Math.PI / 2.0;
